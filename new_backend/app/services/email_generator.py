...
class WebScraper:
    ...
    def __init__(self):
        ...
        # Remove any default proxy settings
        self.session.proxies = {}
        self.session.trust_env = False
        self.session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
        })
        logging.debug("Initialized WebScraper with empty proxies and trust_env=False")

    def extract_info(self, url: str) -> Dict[str, Any]:
        logging.debug("Extracting info from URL: %s", url)
        ...
        try:
            ...
            logging.debug("WebScraper extraction result: %s", result)
            return result
        except Exception as e:
            logging.exception("WebScraper failed to extract info")
            return {"success": False, "error": str(e), "data": {}}

class EmailGenerator:
    def __init__(self, db: Session):
        logging.debug(
            "Initializing AzureOpenAI with api_key: %s..., api_version: %s, azure_endpoint: %s",
            AZURE_OPENAI_API_KEY[:6],  # Don't log full key!
            "2023-12-01-preview",
        ...
        try:
            ...
            logging.debug("AzureOpenAI client initialized successfully")
        except Exception as e:
            logging.error("Failed to initialize AzureOpenAI: %s", str(e), exc_info=True)
            raise
        self.db = db
    ...
    def extract_website_info(self, website_url: str) -> str:
        ...
        if not info["success"]:
            error_msg = f"Failed to extract information: {info.get('error', 'Unknown error')}"
        ...
        logging.debug("Extracted website info: %s", result)
        return result
    ...
    def generate_personalized_email(
        ...
        try:
            logging.debug("Calling AzureOpenAI with prompt: %s", prompt[:500])  # log only the first 500 chars
            ...
            logging.info("Generated and saved email for %s", contact_data.get("Email", "unknown"))
            return email
        except Exception as e:
            logging.error("Failed to generate email: %s", str(e), exc_info=True)
            raise
        ...
        for contact in csv_data:
            try:
                if not contact.get("Email"):
                    logging.warning("Skipping contact: Missing email address")
                    continue
                if not contact.get("First Name") or not contact.get("Last Name"):
                    logging.warning("Skipping contact %s: Missing name information", contact.get('Email'))
                    continue
                email = self.generate_personalized_email(contact, user, template, stage)
                generated_emails.append(email)
            except Exception as e:
                logging.error("Error generating email for %s: %s", contact.get('Email', ''), str(e), exc_info=True)
